...
#define pppoe_hdr_tags(ph)	((struct pppoe_tag *)((u8 *)(ph) + sizeof(struct pppoe_hdr)))
#define pppoe_tag_data(tag)	((char *)((u8 *)(tag) + sizeof(struct pppoe_tag)))
#else
#define pppoe_hdr_tags(ph)	((ph)->tag)
...
static unsigned char *__nat25_find_pppoe_tag(struct pppoe_hdr *ph, unsigned short type)
{
	...
	start_ptr = cur_ptr = (unsigned char *)pppoe_hdr_tags(ph);
...
static int __nat25_add_pppoe_tag(struct sk_buff *skb, struct pppoe_tag *tag)
{
	...
	memmove(((unsigned char *)pppoe_hdr_tags(ph) + data_len), (unsigned char *)pppoe_hdr_tags(ph), ntohs(ph->length));
	ph->length = htons(ntohs(ph->length) + data_len);
	memcpy((unsigned char *)pppoe_hdr_tags(ph), tag, data_len);
	return data_len;
}
